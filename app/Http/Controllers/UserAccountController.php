...
class UserAccountController extends Controller
{
    ...
    {   
        ...
        $lastMonthLabel = $currentTime->subMonth(1)->format('M');
        $twoMonthLabel =  $currentTime->subMonths()->format('M');
        $threeMonthLabel = $currentTime->subMonths()->format('M');

        $chart = Chartjs::build()
                ->name("bar")
                ->type('bar')
                ->size(['width' => 400, 'height' => 200])
                ->labels([$threeMonthLabel, $twoMonthLabel, $lastMonthLabel])
                ->datasets([
                [
                    
                    'data' => [ 2, 1, $lastMonthUsers],
                    'borderRadius' => 10,
                    'borderWidth' => 0,
                    'barPercentage' => 1,
                    'fill' => true,
